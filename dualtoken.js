...
function sign_token({
    ...
    ip_ranges}) {
        ...
        if (full_path !== undefined) {
            ...
            var field = `PathGlobs=${path_globs}`
            tokens.push(field)
            to_sign.push(field)
        }
        else if (url_prefix !== undefined) {
            var field = "URLPrefix=" + Buffer.from(url_prefix).toString('base64url');
            tokens.push(field)
            to_sign.push(field)
        } 
        else {
            throw new Error("User Input Missing: One of `url_prefix`, `full_path` or `path_globs` must be specified");
        }

        // check & parse optional params
        if (start_time !== undefined) {
            var field = `Starts=${parseInt(start_time / 1000)}`
            tokens.push(field)
            to_sign.push(field)
        }
        ...
        if (expiration_time == undefined) {
            var date = new Date();
            var expiration_time = date.setTime(date.getTime() + 1 * 60 * 60 * 1000);
        }
        var field = `Expires=${parseInt(expiration_time / 1000)}`
        tokens.push(field)
        to_sign.push(field)

        if (session_id !== undefined){
            var field = `SessionID=${session_id}`
            tokens.push(field)
            to_sign.push(field)
        }
        ...
        if (ip_ranges !== undefined){
            var field = `IPRanges=${Buffer.from(ip_ranges).toString('base64url')}` 
            tokens.push(field)
            to_sign.push(field)
        }
        ...
        if (algo == "ed25519"){
            ...
            const private_key = crypto.createPrivateKey({
                key: Buffer.concat([Buffer.from("302e020100300506032b657004220420", "hex"),decoded_key]),
                format: "der",
                type: "pkcs8"
              })
            const digest = crypto.sign(null, Buffer.from(to_sign_string), {key: private_key})
            tokens.push("Signature=" + Buffer.from(digest).toString('base64url'))
        }  
        else if (algo == "sha256"){
            const signature = crypto.createHmac('sha256', decoded_key).update(to_sign_string).digest('hex')
            tokens.push("hmac=" + signature)
        }
        else if (algo == "sha1"){
            const signature = crypto.createHmac('sha1', decoded_key).update(to_sign_string).digest('hex')
            tokens.push("hmac=" + signature)
        }
        else {
            throw new Error("Input Missing Error: `signature_algorithm` can only be one of `sha1`, `sha256` or `ed25519`");
        } 
        return tokens.join("~")
}
